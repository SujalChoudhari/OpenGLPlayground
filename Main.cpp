...
int main() {
	...
	glEnableVertexAttribArray(0);					// Enable the Vertex Arttribute Array at index 0 (from start)
	glBindBuffer(GL_ARRAY_BUFFER, 0);				// Bind the Buffer with the location 0 (used in shader source code)
	glBindVertexArray(0);							// Bind the Vertex Array with the location 0.
